...
| 2.2 | 2026-07-23 | Solution Architecture Team | Enabled **multi-part single-turn requests** (e.g. "show my tickets and raise a leave request"). Specialists keep `disallow_transfer_to_parent=True` — verified in ADK 2.5.0 `runners._is_transferable_across_agent_tree`, which keys only off the parent flag, so routing stays non-sticky — but now set `disallow_transfer_to_peers=False`, so a specialist finishes its own part and transfers directly to the sibling that owns the remaining part within the same turn (shared session history; the terminal specialist composes the combined reply). Fixed two blockers this surfaced: `enforce_tool_policy` denied the framework `transfer_to_agent` control as "not in the approved MCP catalog", and `capture_session_identity` dereferenced its `None` response — both now skip the non-MCP transfer control. Deterministic guardrails and conversational confirmation are unchanged; the leave/ticket write still commits only after the employee confirms in a following message. |
